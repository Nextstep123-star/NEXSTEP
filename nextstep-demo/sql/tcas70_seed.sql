...
--
-- ⚠️ idempotent: ลบแถวเดิมที่ "title ตรงกัน" ก่อนเพิ่มใหม่ → รันซ้ำได้ ไม่ซ้ำซ้อน
--    (events.id / news.id เป็น bigint auto-increment จึงไม่ระบุ id — ปล่อยให้ DB gen เอง)
--
-- 📝 หมายเหตุคอลัมน์ที่สคริปต์นี้ใช้ (ถ้าตารางจริงต่างจากนี้ ให้ปรับ):
--    events: (title, event_date, type, color, description)
--    news:   (title, body, category, published_at, is_published)
-- ============================================================

-- ---------- EVENTS: กำหนดการกลาง TCAS70 (ทางการ) ----------
delete from public.events where title in (
  'เปิดระบบ myTCAS — ลงทะเบียน Dek70',
  'รอบ 1 Portfolio — เปิดรับสมัคร',
  'รอบ 1 Portfolio — ประกาศผล',
  'สอบ TGAT / TPAT2-5',
  'สอบ TPAT1 วิชาเฉพาะ กสพท.',
  'สอบ A-Level',
  'รอบ 2 โควตา — ช่วงรับสมัคร',
  'รอบ 3 Admission — รับสมัคร',
  'รอบ 3 Admission — ประกาศผลครั้งที่ 1',
  'รอบ 4 Direct Admission'
);

insert into public.events (title, event_date, type, color, description) values
  ('เปิดระบบ myTCAS — ลงทะเบียน Dek70',        '2026-07-15', 'ลงทะเบียน', 'tertiary',  'ยืนยันตัวตนผ่าน student.mytcas.com ก่อนเข้าสู่การคัดเลือก'),
  ('รอบ 1 Portfolio — เปิดรับสมัคร',            '2026-10-01', 'รับสมัคร',  'primary',   'ยื่นแฟ้มสะสมผลงาน ไม่ใช้คะแนนสอบกลาง'),
  ('รอบ 1 Portfolio — ประกาศผล',                '2027-01-20', 'ประกาศผล',  'primary',   'ประกาศผลผู้ผ่านการคัดเลือกรอบ 1'),
  ('สอบ TGAT / TPAT2-5',                        '2027-01-30', 'สอบกลาง',   'error',     'TGAT ความถนัดทั่วไป และ TPAT2-5 (30 ม.ค.–1 ก.พ. 70)'),
  ('สอบ TPAT1 วิชาเฉพาะ กสพท.',                 '2027-02-13', 'สอบกลาง',   'error',     'วิชาเฉพาะสำหรับกลุ่มแพทย์ (กสพท)'),
  ('สอบ A-Level',                               '2027-03-13', 'สอบกลาง',   'error',     'A-Level 13-15 มี.ค. 70'),
  ('รอบ 2 โควตา — ช่วงรับสมัคร',                '2027-02-15', 'รับสมัคร',  'secondary', 'โควตาพื้นที่/โครงการ'),
  ('รอบ 3 Admission — รับสมัคร',                '2027-04-06', 'รับสมัคร',  'tertiary',  'ใช้คะแนนกลาง TGAT/TPAT + A-Level'),
  ('รอบ 3 Admission — ประกาศผลครั้งที่ 1',      '2027-05-02', 'ประกาศผล',  'tertiary',  'ประกาศผลรอบ 3 ครั้งที่ 1'),
  ('รอบ 4 Direct Admission',                    '2027-05-20', 'รับสมัคร',  'primary',   'รับตรงอิสระ (ถ้ายังมีที่นั่งเหลือ)');

-- ---------- NEWS: ข่าว/ประกาศ TCAS70 ----------
delete from public.news where title in (
  'ทปอ. เปิดระบบ myTCAS ปีการศึกษา 2570 — ลงทะเบียน Dek70 เริ่ม 15 ก.ค. 69',
  'สรุปกำหนดการสอบกลาง TCAS70: TGAT/TPAT 30 ม.ค.–1 ก.พ. 70, A-Level 13-15 มี.ค. 70',
  'TCAS70 คงรูปแบบ 4 รอบ: Portfolio · โควตา · Admission · Direct Admission',
  'กสพท ประกาศใช้ TPAT1 + 7 วิชา A-Level สำหรับสายแพทย์ TCAS70'
);

insert into public.news (title, body, category, published_at, is_published) values
  ('ทปอ. เปิดระบบ myTCAS ปีการศึกษา 2570 — ลงทะเบียน Dek70 เริ่ม 15 ก.ค. 69',
...
-- ---------- CLEANUP: ให้ events/news เหลือแค่ TCAS70 (ลบข้อมูลปีเก่าทิ้ง) ----------
-- ⚠️ ลบทุกแถวที่ "ไม่ใช่ TCAS70" — ถ้ามี event/news อื่นที่อยากเก็บ ให้คอมเมนต์ 2 คำสั่งนี้
delete from public.events where title not in (
  'เปิดระบบ myTCAS — ลงทะเบียน Dek70',
  'รอบ 1 Portfolio — เปิดรับสมัคร',
  'รอบ 1 Portfolio — ประกาศผล',
  'สอบ TGAT / TPAT2-5',
...
delete from public.news where title not in (
  'ทปอ. เปิดระบบ myTCAS ปีการศึกษา 2570 — ลงทะเบียน Dek70 เริ่ม 15 ก.ค. 69',
  'สรุปกำหนดการสอบกลาง TCAS70: TGAT/TPAT 30 ม.ค.–1 ก.พ. 70, A-Level 13-15 มี.ค. 70',
  'TCAS70 คงรูปแบบ 4 รอบ: Portfolio · โควตา · Admission · Direct Admission',
  'กสพท ประกาศใช้ TPAT1 + 7 วิชา A-Level สำหรับสายแพทย์ TCAS70'
...
-- แต่ถ้าอยากเก็บใน DB ด้วย ให้รันส่วนนี้ (key เป็น text ใช้ id เองได้)
-- ============================================================
